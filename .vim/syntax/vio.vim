...
syn keyword     mykeywordsAMA      nnnext
syn keyword     mykeywordsORA      ppost
syn keyword     mykeywordsVERD     PENDING 
syn keyword     mykeywordsAZU      yuhuuu 
syn keyword     mykeywordsROS      SOLVED
syn keyword	vioWOCOLyellow	   email contained
syn keyword	vioWOCOLgreen	   documento contained
"syn region	vioANY		start=+[a-z]\++ end=+.\(ods\|odt\|pdf\)+ contained
"syn region	vioWOCOLdocus	   start=+\(doc\|pdf\):+ end=+.\(ods\|odt\|pdf\)+
"
syn region	vioCOMILLAS start=+"+ end=+" + contains=@viocluster,vioWOCOLyellow,vioWOCOLgreen,@mykeywords
syn match vioEnunciaTagClose "[a-z]*" contained
syn match vioEnunciaTagF2 "\([a-z\|-]\+ \?\)\+ [1-9][0-9]\?" contained
syn match vioEnunciaTagF4 "[a-z]\+ [1-9][0-9]\?:" contained
syn region      vioVIOTAGS	start=+^<+ end=+>+ contains=@viocluster,vioWOCOLyellow,vioWOCOLgreen,@mykeywords,vioEnunciaTagF2,vioEnunciaTagF4
syn region      vioVIOTAGSchap	start=+^</+ end=+>+ contains=vioEnunciaTagClose
syn region      vVIOsuperHI	start=+|+ end=+!|+ contains=@viocluster
syn region      vioQUESTION	start=+¿+ end=+??+ contains=@viocluster
syn match	vCHARS1 	"→" 
syn match   vCHARS2     "⇒ " 
syn match   vCHARS3     "»" 
syn match   vCHARS4     ">>" 
...
syn match       CommentV       "^\#.*" contains=@viocluster
syn region 	commentr start=+\#\#\#+  end=+\#\#\#\#+ contains=@viocluster
syn match       cliv       "^\$.*"
syn match       clivcom     "^☭ .*" contains=hoz
syn match	hoz	"☭ " contained
hi def hoz cterm=bold ctermbg=188 ctermfg=Red 
syn match		vioNumeredLine1	"^[0-9]\+\.	.*"	contains=@viocluster,@mykeywords,@vioGRAMA,@vioHiOneColorCluster
syn match		vioNumeredLine2	"^	[0-9]\+\.	.*"	contains=@viocluster,@mykeywords,@vioGRAMA,@vioHiOneColorCluster
syn match		vioLeteredLine1	"^[A-Z]\+\.	.*"	contains=@viocluster,@mykeywords,@vioGRAMA,@vioHiOneColorCluster
syn match		vioLeteredLine2	"^	[a-z]\+\.	.*"	contains=@viocluster,@mykeywords,@vioGRAMA,@vioHiOneColorCluster
syn match       lsliclivfi "[aa-zz]\+\.[aa-zz]\+" contained 
syn match       viotildedbrac "`\([aa-zz]\+\| \+\)`" contained 
syn match       lslicliv       "^→ .*" contains=lsliclivfi,viotildedbrac,mykeywordsrox,mykeywordsama,mykeywordsverd,mykeywordsazu,mykeywordsros,mykeywordsora extend
syn region	vioLink start=+http[^<]*+ end=+/*+
syn region  vioRefTag start=+<ref>+  end=+</ref>+ contains=vioLink,@Spell extend
syn region	vioGooAsk	start=+[A-Z]\++ end=+???+ contained
syn region  vioGooTag start=+<plog>+  end=+</plog>+ contains=vioGooAsk,@Spell extend


syn region 	vimrcHiOneColorY start=+\[+  end=+\]+ contains=@vioHiOneColorClustear,mykeywordsROX
syn region 	vimrcHiOneColorO start=+ \/\.+  end=+\.\/+ contains=@vioHiOneColorCluster
syn region 	vimrcHiOneColorDO start=+{+  end=+}+ contains=@vioHiOneColorCluster
syn region 	vimrcHiOneColorG start=+\-\.+  end=+\.\-+ contains=@vioHiOneColorCluster
syn region 	vimrcHiOneColorB start=-+\.-  end=-\.+- contains=@vioHiOneColorCluster
syn region 	vimrcHiOneColorR start=+\.\*\*+  end=+\*\*+ contains=@vioHiOneColorCluster
syn region 	vimrcHiOneColorLR start=+\*\.+  end=+\.\*+ contains=@vioHiOneColorCluster

""""""""ESPECIALES VIO-LOG
"syn match       vintro       "^.*::"
syn	region	vioTAGVlog	start=+^</\?[0-9]+ end=+>+ contains=vioTAGVlogdir
syn match	vioTAGVlogdir	"[^<]/[^>]*" contained

hi def link vioTAGVlog		vimrcHiOneColorY
hi def link vioTAGVlogdir	vimrcHiOneColorG


syn cluster	vioHiOneColorCluster	contains=vimrcHiOneColorY,vimrcHiOneColorO,vimrcHiOneColorG,vimrcHiOneColorB,vimrcHiOneColorR,vimrcHiOneColorLR
syn cluster     viocluster     contains=cStatement,vioVIOTAGS,vioVIOTAGSchap,CommentV,CommentR,cliV,@vioHiOneColorCluster,vDest,vioRefTag
syn cluster 	mykeywords	contains=mykeywordsORA,mykeywordsROS,mykeywordsROX,mykeywordsAMA,mykeywordsVERD,mykeywordsAZU
syn case match

hi def clivcom term=bold ctermbg=188 ctermfg=DarkGreen
hi def link vioCOMILLAS viogramaDefITALIC
"hi def  link vCHARS1    SignColumn 
"hi def  link vCHARS2    SignColumn 
"hi def  link vCHARS3    SignColumn 
"hi def  link vCHARS4    SignColumn 
hi def  vCHARS1 term=bold ctermfg=Cyan
hi def  vCHARS2 term=bold ctermfg=Cyan
hi def  vCHARS3 term=bold ctermfg=Cyan
hi def  vCHARS4 term=bold ctermfg=Cyan
hi def vioQUESTION ctermfg=121 ctermfg=LightGreen
hi def vVIOsuperHI term=bold ctermbg=188 ctermfg=Blue
hi def vioNumeredLine1 term=bold ctermfg=81 guifg=Cyan
hi def vioNumeredLine2 ctermfg=87 
hi def vioLeteredLine1 ctermfg=231 
hi def vioLeteredLine2 ctermfg=192 
hi def vioLink term=bold ctermfg=86 guifg=Blue
hi def vioGooAsk term=bold ctermfg=86 ctermfg=DarkYellow
hi def CommentV term=bold ctermfg=87 guifg=Cyan
hi def CommentR term=bold ctermfg=159 guifg=Cyan
hi def  vioVIOTAGS term=bold ctermfg=165 guifg=Purple
hi def  vioVIOTAGSchap term=bold ctermfg=165 guifg=Purple
hi def	link vioEnunciaTagF2 vimrcHiOneColorY
hi def	link vioEnunciaTagF4 vimrcHiOneColorY
hi def	link vioEnunciaTagClose vimrcHiOneColorY
hi def  cliV term=bold ctermbg=188 ctermfg=DarkGreen
hi def  vioTildedBrac term=bold ctermbg=188 ctermfg=Blue
hi def  lslicliVfi term=bold ctermbg=188 ctermfg=DarkBlue
hi def  lslicliV term=bold ctermbg=188 ctermfg=DarkYellow
hi def  vimrcHiOneColorY term=bold ctermfg=Yellow
hi def  vimrcHiOneColorDO term=bold ctermbg=215 ctermfg=21
hi def  vimrcHiOneColorO term=bold ctermfg=215
hi def  vimrcHiOneColorG term=bold ctermfg=LightGreen
hi def  vimrcHiOneColorB term=bold ctermfg=LightBlue
hi def  vimrcHiOneColorR term=bold ctermfg=Red
hi def  vimrcHiOneColorLR term=bold ctermfg=LightRed
...
hi def CommentR ctermfg=cyan guifg=#00ffff
hi def vioRefTag ctermfg=green guifg=#00ffff
hi def vioGooTag ctermfg=blue guifg=#00ffff
...
hi def mykeywordsORA	term=bold ctermfg=216
hi def mykeywordsAMA	term=bold ctermfg=226
...
hi def link vioWOCOLyellow	vimrcHiOneColorY
hi def link vioWOCOLgreen	vioQUESTION
...
syn keyword adverbiosCalidad  bien mal mejor peor regular despacio deprisa tal como adrede claro exacto obvio inclusive además asimismo únicamente especialmente incluso viceversa siquiera inicialmente finalmente Bien Mal Mejor Peor Regular Despacio Deprisa Tal Como Adrede Claro Exacto Obvio Inclusive Además Asimismo Únicamente Especialmente Incluso Viceversa Siquiera Inicialmente Finalmente
syn keyword adverbiosCantidad más menos muy mucho poco apenas algo casi aproximadamente exactamente bastante justo demasiado etcétera solo solamente tan tanto todo nada cómo cuándo cuánto cuál cuáles dónde Más Menos Muy Mucho Poco Apenas Algo Casi Aproximadamente Exactamente Bastante Justo Demasiado Etcétera Solo Solamente Tan Tanto Todo Nada Cómo Cuándo Cuánto Cuál Cuáles
syn keyword adverbiosPosibilidad siempre nunca jamás también tampoco quizá quizás acaso fácilmente difícilmente probablemente posiblemente seguramente Siempre Nunca Jamás También Tampoco Quizá Quizás Acaso Fácilmente Difícilmente Probablemente Posiblemente Seguramente 
...
syn keyword pronombresPersonales yo tú vos usted él ella ello nosotros nosotras vosotros vosotras ustedes ellos ellas mí conmigo ti contigo sí consigo me te le lacual lo se nos os Yo Tú Vos Usted Él Ella Ello Nosotros Nosotras Vosotros Vosotras Ustedes Ellos Ellas Mí Conmigo Ti Contigo Sí Consigo Me Te Le Lacual Lo Se Nos Os
...
syn keyword viogramaDobleSyn y e o Y E O que porque sí Sí SÍ no No NO
"2//// GRUPOS DE SYNTAX ==> UNDERLINED ITALIC; hi def link
"UNDERLINED
syn region vioMAYUSb start=+[A-Z]\++ end=+ + contained
syn region vioMAYUSi start=+[A-Z]\++ end=+ + contained
syn region vioMAYUSu start=+[A-Z]\++ end=+ + contained
syn cluster vioMAYUS contains=vioMAYUSb,vioMAYUSi,vioMAYUSu
hi  def vioMAYUS  ctermfg=229
hi  def vioMAYUSb cterm=bold ctermfg=226
hi  def vioMAYUSi cterm=italic  ctermfg=231
hi  def vioMAYUSu cterm=underline ctermfg=226
syn region viogramaDefUNDERLINED start=+<u>+ end=+</u>+ contains=@viocluster,@mykeywords,@vioGRAMA,@vioHiOneColorCluster,vioMAYUSu
syn region viogramaDefITALIC start=+<i>+ end=+</i>+ contains=@viocluster,@mykeywords,@vioGRAMA,@vioHiOneColorCluster,vioMAYUSi
syn region viogramaDefBOLD start=+<b>+ end=+</b>+ contains=@viocluster,@mykeywords,@vioGRAMA,@vioHiOneColorCluster,vioMAYUSb
...
hi  def adverbiosOtros ctermfg=229
...
hi  def pronombresIndefinidos ctermfg=49 
...
syn cluster	vioGRAMA		contains=@viogramaUNDERLINED,@viogramaITALIC,viogramaDobleSyn
""""HASTAAQUIAMETSA"
syn match       vINTRO  "^.*::"
syn match       Vestre  "★"
